...
<script type="text/javascript"> 
    ...
    var dataPurchaseOrder = '<?php  echo json_encode($purchaseOrder) ;?>'
    ...
    $(document).ready(function () {
        ...
        dataPurchaseOrder =  $.parseJSON(dataPurchaseOrder)
        purchase_id =  dataPurchaseOrder.id
        var invoice_form_type = dataPurchaseOrder.type 

        // change date formated
        var transaction_date = dataPurchaseOrder.transaction_date
        var delivery_date = dataPurchaseOrder.delivery_date ? dataPurchaseOrder.delivery_date : "-"
        transaction_date = transaction_date.split('-');
        delivery_date = delivery_date.split('-');
        var reversedTransactionDate = transaction_date[2] + '-' + transaction_date[1] + '-' + transaction_date[0]
        var reversedDeliveryDate = delivery_date[2] + '-' + delivery_date[1] + '-' + delivery_date[0]
       
        // Assign dan passing data value into elements
        var vendor_id = dataPurchaseOrder.vendor_id
        var warehouse_id =  dataPurchaseOrder.warehouse_id
        var discount = ((dataPurchaseOrder.discount_invoice / 100) * dataPurchaseOrder.subtotal)

        getVendor(vendor_id)
        getVendor()
        getWarehouse(warehouse_id)
        getInvoiceCategory(dataPurchaseOrder.category_invoice_id)
        
        loadDetailPurchaseOrders(purchase_id, invoice_form_type)

        $("#id").val(dataPurchaseOrder.id)
        $("#vendor_reference").val(dataPurchaseOrder.vendor_reference)
        $("#vendor_phone").val(dataPurchaseOrder.vendor_phone)
        $('#date').val(reversedTransactionDate).prop("disabled", true);
        // $('#due_date').val(reversedDeliveryDate).prop("disabled", true);
        $("#invoice_number").val(dataPurchaseOrder.purchaseorder_number).prop("disabled", true)
        $("#batch_number").val(dataPurchaseOrder.batch_number)
        $("#day").val(dataPurchaseOrder.day)
        $("#note").val(dataPurchaseOrder.note)
        $("#subtotal").val(dataPurchaseOrder.subtotal)
        $("#tax").val(dataPurchaseOrder.tax)
        $("#discount_invoice").val(dataPurchaseOrder.discount_invoice)
        $("#discount").val(dataPurchaseOrder.discount)
        $("#additional_char").val(dataPurchaseOrder.additional_char)
        $("#grand_total").val(dataPurchaseOrder.grand_total)
        $("#balance_due").val(dataPurchaseOrder.balance_due)
    ...
    function loadDetailPurchaseOrders(purchase_id = null, invoive_form_type=null){
       $.ajax({
            type: "POST",
            url: "/api/purchase/order/detail",
            data: {
